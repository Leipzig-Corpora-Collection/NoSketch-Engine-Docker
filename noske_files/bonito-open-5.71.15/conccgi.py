...
class ConcCGI (UserCGI):
    ...
    def freqdist(self, wordlist = [], diaattr='word', ctx='0', threshold=0):
        def pfrq(diaattr, ctx, item, wlattr, period_sizes, size_limit, exclude_period_names):
            # Word per period frequencies
            fcrit = ' '.join([onelevelcrit('', diaattr, ctx, 1,'rc', '', 'e')])
            ml = '.' not in diaattr

            try:
                freqs = self.freqs([fcrit], 0, self.freq_sort, ml)

                # adjusment for concordances exeeding 10M hits
                k = 1
                if freqs['fullsize'] > freqs['concsize']:
                    k = freqs['fullsize'] / freqs['concsize']

                wl_freqs = {}
                for i in freqs['Blocks'][0]['Items']:
                    wl_freqs[i['Word'][0]['n']] = {'frq': round(i['frq'] * k, 0), 'rel_frq': round(i['reltt'] * k, 0),
                                                   'period_size': period_sizes.get(i['Word'][0]['n'], 0)}

                # filter according to the threshold
                result_items = {}
                removed_items = {}
                for period_name, period_size in period_sizes.items():
                    if wl_freqs.get(period_name):
                        if period_size < size_limit or period_name in exclude_period_names:
                            removed_items[period_name] = wl_freqs.pop(period_name)
                        else:
                            result_items[period_name] = wl_freqs.pop(period_name)
                    else:
                        removed_items[period_name] = {'frq': 0, 'rel_frq': 0, 'period_size': period_size}

                return {wlattr: item, 'freqdist': result_items, 'removed_freqdist': removed_items,
                        'average_norm': average_size, 'norm_limit': size_limit, 'k': k, 'random': 1 if freqs['concsize'] < freqs['fullsize'] else 0}
            except:
                return {wlattr: item, 'error': 1, 'error_msg': f'Error in the query: {self.concordance_query}'}

        from butils import escape_nonwild

        period_sizes = {}
        exclude_period_names = ['===NONE===', '']

        # ========================================
        # get all possible diachronic values
        # ========================================
        sc = self._corp()
        wl = sc.get_attr(diaattr)
        try:
            sortfreq = wl.get_stat('token:l')
        except corplib.manatee.FileAccessError:
            bgjob = corplib.compute_freqfile (self._user, sc, diaattr, 'token:l', self.get_url())
            if bgjob: return bgjob_info(bgjob)

        result_list, _, _ = corplib.manatee.wordlist(wl, '.*', [], sortfreq,
                                                     [], [], 0, 0, 1000000, None)
        for p in result_list:
            p_name, p_size = p.split()
            if p_name not in exclude_period_names:
                period_sizes[p_name] = float(p_size)

        average_size = sum(period_sizes.values()) / len(period_sizes.keys())
        size_limit = average_size * float(threshold)
        self.fmaxitems = len(result_list)

        # ========================================
        # split results according threshold
        # ========================================
        if not wordlist and self.concordance_query:
            yield pfrq(diaattr, ctx, self.concordance_query, 'q', period_sizes, size_limit, exclude_period_names)

        elif wordlist:    
            for item in wordlist:
                # compute frequencies for each item in wordlist/all periods
                self.concordance_query = [{'queryselector': 'cqlrow',
                                           'cql': '[%s="%s"]' % (self.wlattr, escape_nonwild(item)),
                                           'default_attr': self.wlattr, 'random': 1}]
                yield pfrq(diaattr, ctx, item, self.wlattr, period_sizes, size_limit, exclude_period_names)
        else:
            yield {'error': 1, 'error_msg': f'Need to specify wordlist or concordance_query in the request.'}
    ...
    def wordlist (self, wlpat='.*', wltype='simple', corpname='', usesubcorp='',
        ...
        if self.wlattr == 'WSCOLLOC':
            ...
            if "." in self.wlattr:
                ...
                multisep = sc.get_conf('%s.MULTISEP' % self.wlattr)
                hierarch = sc.get_conf('%s.HIERARCHICAL' % self.wlattr)
                if multisep: # only values without MULTISEP
                    wlpat = ('(?!.*%s)' % re.escape(multisep)) + wlpat
                if hierarch:
                    result['show_hierarchical_switch'] = 1
                    if hierarchical_top_only: # only top-hierarchy values
                        wlpat = ('(?!.*%s)' % re.escape(hierarch)) + wlpat
    ...
    def corp_info(self, gramrels=0, corpcheck=0, registry=0, subcorpora=0,
        ...
        compiled = out['sizes'].get('tokencount', 0) > 1
    ...
    def extract_keywords(self, ref_corpname='', simple_n=1.0, reldocf=1,
        ...
        if alnum:
            if attr == "TERM":
                filters.append(self.re_alnum_terms)
            else:
                filters.append(self.re_alnum)
        if onealpha:
            filters.append(self.re_onealpha)
