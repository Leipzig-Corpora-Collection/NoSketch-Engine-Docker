...
class WordlistStoreClass extends FeatureStoreMixin {
    constructor(){
        ...
        this.urlOptions = ["lpos", "find", "keyword", "filter", "viewAs",
            ...
            "showtimelineabs", "timelinesthreshold", "hierarchical_top_only"]
        ...
        if(this.timelineWordsToLoad.length){
            ...
            this.timelineRequest = SSEConnection.get({
                ...
                data: {
                    ...
                    ctx: "0~0>0",
                    diaattr: this.data.diaattr || this.corpus.diachronic[0].value,
                    wlattr: wlattr,
                    sse: 1,
    ...
    setCorpusDefaults(){ //overrides default
        ...
        if(this.corpus){
            ...
            this.defaults.diaattr = this.data.diaattr
        }
        if(attr){
            if(!attr.lc){
                this.data.wlicase = false
    ...
    getRequestData(){ //overrides default
        ...
        if(!this.data.histid){
            ...
            if(this._isTextTypeAnalysis()){
                ...
                data.hierarchical_top_only = this.data.hierarchical_top_only
            } else {
    ...
    getWlpat(){
        ...
        ;["startingWith", "containing", "endingWith", "matchingRegex"].forEach((filter) => {
    ...
    areTimelinesAvailable(){
        return this.corpus.diachronic.length
                && !this.data.histid
                && !this.data.wlstruct_attr1
                && !this._isTextTypeAnalysis()
    }

    areTimelinesDisplayed(){
        return this.areTimelinesAvailable() && this.data.showtimelines
    }
    ...
    _setDataFromUrl(query){
        ...
        this.data.criteria.forEach(criterion => {
            if(criterion.filter == "regex"){
                criterion.filter ="matchingRegex"
            }
        })
