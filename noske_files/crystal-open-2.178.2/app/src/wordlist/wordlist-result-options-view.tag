    ...
    <div class="row">
      <div class="col l4 m6 s12 dividerRight">
        <ui-switch label-id="singleColumn"
                name="onecolumn"
                disabled={!store.hasBeenLoaded || data.isLoading || store.areTimelinesDisplayed()}
                riot-value={data.onecolumn || store.areTimelinesDisplayed()}
    ...
    <script>
        ...
        isAttrDisabled(attr){
            ...
            return !this.store.hasBeenLoaded
                    ...
                    || (isStruct && attr.value == "relfreq" && this.data.raw.concsize <= this.data.raw.fullsize)
        }
    </script>
